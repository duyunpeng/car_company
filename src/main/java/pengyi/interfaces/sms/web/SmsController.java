package pengyi.interfaces.sms.web;
...
public class SmsController extends BaseController {
    ...
    @ResponseBody
    public JsonMessage sendRegister(String userName) {
        JsonMessage jsonMessage = new JsonMessage();
        try {
            if (!redisService.exists(userName)) {
                if (null != companyService.searchByUserName(userName)) {
                    jsonMessage.setCode("1");
                    jsonMessage.setMessage("该号码已被注册!");
                    jsonMessage.setData(null);
                } else {
                    String code = CoreStringUtils.randomNum(6);
                    smsSender.send(userName, code, SmsTemplate.REGISTER);
    ...
    public JsonMessage sendResetPassword(String userName) {
        ...
        try {
            if (!redisService.exists(userName)) {
                if (null == companyService.searchByRole(userName)) {
                    jsonMessage.setCode("1");
